...
const copies = {
  ko: { title: '투숙 가능한 객실을 선택해 주세요', description: '선택한 기간 전체에 투숙 가능한 객실입니다.', loading: '객실 일정을 확인하고 있습니다.', empty: '선택한 기간 전체에 투숙 가능한 객실이 없습니다.', partialTitle: '선택 기간과 일부만 겹치는 객실', partialDescription: '투숙 가능한 날짜가 많이 겹치는 객실부터 표시합니다.', availableNights: '{total}박 중 {available}박 가능', cancel: '취소', continue: '선택한 객실로 계속하기' },
  en: { title: 'Select an available room', description: 'These rooms are available for your entire stay.', loading: 'Checking room availability.', empty: 'No room is available for the entire selected stay.', partialTitle: 'Rooms available for part of your stay', partialDescription: 'Rooms with more matching nights are shown first.', availableNights: '{available} of {total} nights available', cancel: 'Cancel', continue: 'Continue with selected room' },
} as const;
          ...
          <>
            ...
            {partiallyAvailableRooms.length ? (
              <aside className="partialRoomSection" aria-labelledby="partial-room-title">
                ...
                <ul>{partiallyAvailableRooms.map((room) => <li key={room.roomNo}><strong>{room.roomNo}</strong><span>{copy.availableNights.replace('{available}', String(room.availableNights)).replace('{total}', String(room.requestedNights))}</span></li>)}</ul>
              </aside>
            ) : null}
          </>
        )}
