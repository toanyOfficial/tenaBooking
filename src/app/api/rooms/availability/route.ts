...
import { getRoomAvailability, RoomAvailabilityError } from '@/features/booking/services/roomAvailabilityService';
...
function getAvailabilityError(error: unknown): { message: string; errorCode: string } {
  if (error instanceof RoomAvailabilityError) return { message: error.message, errorCode: error.code };
  ...
  if (databaseError.code === 'ER_NO_SUCH_TABLE') return { message: 'reservation_calendar_snapshot 테이블을 찾을 수 없습니다.', errorCode: databaseError.code };
  ...
  if (databaseError.code === 'ER_BAD_FIELD_ERROR') return { message: 'reservation_calendar_snapshot 테이블의 컬럼 구성이 조회 조건과 맞지 않습니다.', errorCode: databaseError.code };
